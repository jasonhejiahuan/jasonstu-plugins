...
function normalizePlannedQueries(value, scopes, limit, question, round) {
  const normalized = [];
  ...
  const suffixes =
    round === 0
      ? ["关键事实与最新进展", "权威研究与原始证据", "争议与反方观点", "统计数据与案例"]
      : ["尚未解决的证据缺口", "相反结论与局限", "最新原始资料", "可验证数据"];
...
function mergeAnswerSources(response, sources, sourceKeys) {
  const candidates = [
  ...
  for (const item of candidates) {
    ...
    if (!url || sourceKeys.has(url)) continue;
    sourceKeys.add(url);
    sources.push({
...
function auditCitationIds(report, sources) {
  ...
  return {
    valid: invalidIds.length === 0 && !missingCitations,
...
function buildSynthesisPrompt({ question, language, sources, readerEvidence, draft, critique }) {
...
export async function runDeepResearch(client, input = {}) {
  ...
  if (depth === "deep") {
    const draftResponse = await client.answer({
      question: buildSynthesisPrompt({ question, language, sources, readerEvidence }),
      scope: "webpage",
      model,
      format: "simple",
      stream: false,
  ...
  mergeAnswerSources(finalResponse, sources, sourceKeys);
  let answer = textFromAnswer(finalResponse).trim();
  let citationValidation = auditCitationIds(answer, sources);
  let citationRepairAttempted = false;
  if (!citationValidation.valid && sources.length > 0) {
    ...
    mergeAnswerSources(repairResponse, sources, sourceKeys);
    const repaired = textFromAnswer(repairResponse).trim();
  ...
  if (!citationValidation.valid) {
    const warning = citationValidation.missingCitations
      ? "Citation validation warning: the generated report did not cite the available source IDs."
      : `Citation validation warning: unknown source IDs remain (${citationValidation.invalidIds.join(", ")}).`;
    answer = `> **${warning}**\n\n${answer}`;
  }
