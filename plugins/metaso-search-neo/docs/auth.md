...
Windows selects an available PowerShell 7 runtime first, including its standard installation location when it is absent from `PATH`. A nonsecret, read-only capability probe must return the exact success marker; exit code zero with no output is not accepted. Windows PowerShell 5 is considered only if the PowerShell 7 probes fail. Once selected, the runtime is reused: a failed ACL application or verification never retries through another engine or relaxes the permission checks. A new current-user-only descriptor is applied with `Set-Acl` only to newly created empty storage objects; existing unsafe files and directories are rejected, not repaired. [Microsoft Set-Acl reference](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.security/set-acl?view=powershell-7.5)

Each Windows PowerShell child removes inherited `PSModulePath` so its runtime can construct compatible module paths. Without this, launching Node from PowerShell 7 can make Windows PowerShell 5's `Get-Acl` fail to load. This changes only the child environment, not user settings, security software or ACL checks. If no runtime passes the probe, private-file setup stops; the plugin does not install PowerShell or attempt to bypass host policy. [Microsoft module-path guidance](https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_psmodulepath?view=powershell-7.6#starting-windows-powershell-from-powershell-7)
...
On the same date, an authorized physical Windows host with PowerShell 7.6.6 and portable Node.js 22.23.2 passed 59 storage/auth/client tests (two POSIX-only tests skipped) and the MCP initialization/tool-list/status smoke check. This included native rejection of an Everyone-read ACE, exact fresh-object ACL initialization, concurrent writes and credential replacement. PowerShell 5 was unusable on that host; successful PowerShell 7 selection required no changes to security software, system installation or `PATH`. Only synthetic test credentials were used on Windows, not a real MetaSo Key.
